(() => {
    ...
    (() => {
        ...
        BasketCalculate();
        function BasketAdd() {
            let basket = document.querySelector(".header-bottom__basket-quantity");
            let basketCounter = 0;
            function basketHide() {
                basketCounter < 1 ? basket.style.visibility = "hidden" : basket.style.visibility = "";
            }
            basketHide();
            if (document.querySelector(".card-btn")) document.querySelector("main").addEventListener("click", (e => {
                if (e.target.closest(".card-btn._active")) basketCounter--; else if (e.target.closest(".card-btn")) basketCounter++;
                e.target.closest(".card-btn")?.classList.toggle("_active");
